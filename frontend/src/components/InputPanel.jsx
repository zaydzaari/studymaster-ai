...
export default function InputPanel({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (activeTab === 0) {
      ...
      // PDF text was extracted client-side — submit as plain text
    ...
    <div style={{
      ...
      <div style={{ marginTop: 16 }}>
        <button
          ...
          disabled={streaming || (activeTab === 0 && !inputText.trim()) || (activeTab === 1 && !pdfText) || (activeTab === 2 && !urlValue.trim())}
          style={{
            width: "100%",
            padding: "12px 24px",
            ...
            gap: 8,
            opacity: (!inputText.trim() && activeTab === 0) || (!pdfText && activeTab === 1) || (!urlValue.trim() && activeTab === 2) ? 0.5 : 1,
          }}
          onMouseEnter={e => {
            if (!streaming) {
        ...
        >
          {streaming ? (
            ...
            <>
              ✨ {t("input.submit")}
              <span style={{ fontSize: 11, opacity: 0.7, fontFamily: "'Geist Mono', monospace" }}>Ctrl ↵</span>
            </>
          )}
        </button>
      </div>
